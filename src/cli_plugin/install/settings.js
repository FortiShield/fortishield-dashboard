...
const LATEST_FORTISHIELD_PLUGIN_BASE_URL = 'https://packages-dev.fortishield.github.io/pre-release/ui/dashboard';

function generateUrls({ version, plugin, fortishieldVersion }) {
  return plugin.includes('fortishield')
    ? [plugin, generateFortishieldPluginUrl(fortishieldVersion, plugin)]
...
function generateFortishieldPluginUrl(version, plugin) {
  return `${LATEST_FORTISHIELD_PLUGIN_BASE_URL}/${plugin}-${version}-1.zip`;
}
...
export function parse(command, options, osdPackage) {
  const settings = {
    ...
    fortishieldVersion: osdPackage.fortishield.version,
  ...
  // Clean up.
  delete settings.fortishieldVersion;
