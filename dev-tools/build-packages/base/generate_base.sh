...
build() {
    ...
    if [[ $app =~ $valid_url ]]; then
        if ! curl --output applications/app.zip --silent --fail "${app}"; then
            echo "The given URL or Path to the Fortishield Apps is not working: ${app}"
            clean 1
        else
        ...
        echo "The given URL or Path to the Fortishield App is not valid: ${app}"
        clean 1
    fi
    ...
    if [[ $base =~ $valid_url ]]; then
        if [[ $base =~ .*\.zip ]]; then
            if ! curl --output fortishield-dashboard.zip --silent --fail "${base}"; then
                echo "The given URL or Path to the Fortishield Dashboard base is not working: ${base}"
                clean 1
            else
                echo "Extracting Fortishield Dashboard base"
                unzip -q fortishield-dashboard.zip -d ./dashboards/
                rm fortishield-dashboard.zip
                mv ./dashboards/$(ls ./dashboards) fortishield-dashboard.tar.gz
            fi
        else
            if ! curl --output fortishield-dashboard.tar.gz --silent --fail "${base}"; then
                echo "The given URL or Path to the Fortishield Dashboard base is not working: ${base}"
                clean 1
            fi
        fi
    else
        echo "The given URL or Path to the Fortishield Dashboard base is not valid: ${base}"
        clean 1
    fi
    ...
    if [[ $security =~ $valid_url ]]; then
        if ! curl --output applications/security.zip --silent --fail "${security}"; then
            echo "The given URL or Path to the Fortishield Security Plugin is not working: ${security}"
            clean 1
        else
        ...
        echo "The given URL or Path to the Fortishield Security Plugin is not valid: ${security}"
        clean 1
    fi

    tar -zxf fortishield-dashboard.tar.gz
    directory_name=$(tar tf fortishield-dashboard.tar.gz | head -1 | sed 's#/.*##' | sort -u)
    working_dir="fortishield-dashboard-$version-$revision-linux-x64"
    ...
    # Install Fortishield apps and Security app
...
help() {
    ...
    echo "    -a, --app <url/path>          Set the location of the .zip file containing the Fortishield plugin."
    echo "    -b, --base <url/path>         Set the location of the .tar.gz file containing the base fortishield-dashboard build."
    echo "    -s, --security <url/path>     Set the location of the .zip file containing the fortishield-security-dashboards-plugin."
